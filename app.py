...
# --- Configuration ---
# Use Streamlit secrets if available, else fallback to localhost
                ...
                try:
                    ...
                    if e.response is not None:
                        try:
                            ...
                            pass
                    st.error(f"Login failed: {detail or 'Connection Error'}")
            else:  # Registration
                try:
                    res = requests.post(
                        f"{API_URL}/register",
                    ...
                    if e.response is not None:
                        try:
                            ...
                            pass
                    st.error(f"Registration failed: {detail or 'Connection Error'}")
...
def show_chat_view():
    ...
    st.sidebar.button(
        "➕ New Chat", use_container_width=True, on_click=new_chat_callback
    )
    ...
    for convo in st.session_state.conversations:
        ...
        st.sidebar.button(
            ...
            on_click=select_conversation_callback,
            args=(convo_id,),
        )
    ...
    if prompt := st.chat_input("Ask anything medical..."):
        ...
        with st.spinner("Thinking..."):
            ...
            try:
                res = requests.post(
                    f"{API_URL}/chat", json=payload, headers=get_auth_headers()
                )
                res.raise_for_status()
                data = res.json()
...
def main():
    init_session_state()
    st.markdown(
        "<h1 style='text_align: center;'>🩺 MediGuide</h1>", unsafe_allow_html=True
    )
